...
uses
  ...
  Vcl.Edge, Winapi.ActiveX, Winapi.WebView2, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.DApt.Intf, Data.DB, Vcl.Grids, Vcl.DBGrids,
  FireDAC.Stan.StorageXML, FireDAC.Comp.DataSet, FireDAC.Comp.Client;
...
begin
  peso:=DataSet.FieldByName('peso').AsInteger;
end;

procedure TfrmMain.FDMemTable1NewRecord(DataSet: TDataSet);
begin
DataSet.FieldByName('UF').AsString:='AM';
  DataSet.FieldByName('CIDADE').AsString:='MANAUS';
  DataSet.FieldByName('PAIS').AsString:='BRASIL';
  DataSet.FieldByName('PESO').AsInteger:=peso+1;
end;
...
begin
 ...
 raiz:='https://www.google.com/maps/dir/';
 parametros:='';
 ...
 begin
   ...
   parametros:=parametros+FDMemTable1local.AsString+'+'+FDMemTable1endereco.AsString+'+'+
                          FDMemTable1bairro.AsString+'+'+FDMemTable1cidade.AsString+'+'+FDMemTable1uf.AsString
                           +'+'+FDMemTable1pais.AsString+'/';
   DataSource1.DataSet.Next;

 end;

 EdgeBrowser.Navigate(raiz+parametros);
...
begin
  FDMemTable1.SaveToFile('rota.xml');

end;

procedure TfrmMain.Button3Click(Sender: TObject);
begin
FDMemTable1.LoadFromFile('rota.xml');
end;
